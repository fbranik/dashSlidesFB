...
modelScenario = "mainModel"

arisDatasetToTrain = pd.read_csv(f'assets/{modelScenario}/mainModelData.csv', sep=',')

importancesRead = pd.read_csv(f'assets/{modelScenario}/mainModelImportances.csv', sep=',')
        ...
        children=[
                ...
                dbc.Row([
                        dbc.Col([
                                ...
                                dmc.Select(
                                        id='colorMeasurementsComparisonMainModel',
...
@app.callback(
        Output("graphMeasurementsComparisonMainModel", "figure"),
        Input("colorMeasurementsComparisonMainModel", "value"),
)
def generateMeasurementsComparisonChart(colorMeasurementsComparison):
    ...
    figMeasurementsComparison.write_image(
            mediaFolder + f"main_{colorMeasurementsComparison.replace(' ', '_')}_ModelVsActual.pdf")
    sleep(4)
    figMeasurementsComparison.write_image(
            mediaFolder + f"main_{colorMeasurementsComparison.replace(' ', '_')}_ModelVsActual.pdf")
    figMeasurementsComparison.update_layout(
...
@app.callback(
        Output("featureImportancescolorMainModel", "children"),
        Input("colorMeasurementsComparisonMainModel", "value"),
)
